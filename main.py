...
def recoveryScreen(key):
    ...
    window.geometry('250x125')
    lbl = Label(window, text="Save this key for account recovery")
    lbl.config(anchor=CENTER)
    lbl.pack()
...
def vaultScreen():
    ...
    lbl = Label(window, text="Password Vault by KryTax")
    lbl.grid(column=1)

    btn = Button(window, text="+ADD Passwords", command=addEntry)
